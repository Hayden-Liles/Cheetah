...
mod parser_comprehensive_tests {
    ...
    mod integration_tests {
        ...
        fn test_complex_program() {
            // A more complex program that combines multiple language features
            assert_parses(
                "
# Class definition with inheritance and type annotations
class Calculator(BaseCalculator):
    ...
    @classmethod
    def from_string(cls, precision_str: str) -> 'Calculator':
        return cls(int(precision_str))
...
def main():
    ...
    try:
        expressions = [
            ...
            \"3 - 4\",
            \"5 * 6\",
            \"7 / 0\"  # This will raise an error
        ]
        
        results = []
        for expr in expressions:
            try:
                if (result := calc.compute(expr)) is not None:
