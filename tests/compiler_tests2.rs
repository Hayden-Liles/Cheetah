...
fn test_number_compilation() {
    ...
    assert_eq!(int_val.into_int_value().get_zero_extended_constant(), Some(42));
...
fn test_constant_compilation() {
    ...
    assert!(matches!(true_type, Type::Bool));
    assert!(true_val.is_int_value());
    assert_eq!(true_val.into_int_value().get_zero_extended_constant(), Some(1));
    ...
    assert!(matches!(false_type, Type::Bool));
    assert!(false_val.is_int_value());
    assert_eq!(false_val.into_int_value().get_zero_extended_constant(), Some(0));
    ...
    assert!(none_val.into_pointer_value().is_null());
}
...
fn test_unary_operations() {
    ...
    assert!(matches!(neg_type, Type::Int));
    assert!(neg_val.is_int_value());
}
...
fn test_binary_operations() {
    ...
    assert!(matches!(add_type, Type::Int));
    assert!(add_val.is_int_value());
    ...
    assert!(matches!(mul_type, Type::Int));
    assert!(mul_val.is_int_value());
    ...
    assert!(matches!(mixed_type, Type::Float));
    assert!(mixed_val.is_float_value());
}
...
fn test_variable_references() {
    ...
    assert!(matches!(var_type, Type::Int));
    assert!(var_val.is_int_value());
}