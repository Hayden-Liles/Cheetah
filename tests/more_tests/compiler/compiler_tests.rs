...
fn test_llvm_type_generation() {
    ...
    // With BoxedAny, all types are represented as pointers
    assert!(llvm_int.is_pointer_type());
    assert!(llvm_float.is_pointer_type());
    assert!(llvm_bool.is_pointer_type());
    assert!(llvm_string.is_pointer_type());

    // All types should have the same pointer type
    let ptr_type = llvm_int.into_pointer_type();
    assert_eq!(ptr_type.get_address_space(), inkwell::AddressSpace::default());
}
