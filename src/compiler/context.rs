...
impl<'ctx> CompilationContext<'ctx> {
    ...
    // ---------------------------------------------------------------------------
    // Canonical Boxing API
    // ---------------------------------------------------------------------------

    /// Wrap `val` of static `ty` into a real BoxedAny*.
    /// Returned value is a pointer; returned type is always `Type::Any`.
    pub fn box_value(
        &mut self,
        ...
        let f = self.module
            .get_function(fname)
            .ok_or_else(|| format!("{} missing in module", fname))?;

        let call_result = self.builder
            .build_call(f, &[arg], "box_val")
            .map_err(|e| format!("Failed to build call: {}", e))?;

        let basic_value = call_result.try_as_basic_value().left()
            .ok_or_else(|| "boxing call returned void".to_string())?;

        Ok(basic_value)
    }

    /// – If boxed mode is ON and `ty != Any`, box the value.
    ...
    // -----------------------------------------------------------------------
    // Tiny variable helpers that _always_ do the boxing/unboxing dance.
    // -----------------------------------------------------------------------

    /// Ensure variable exists; create slot if needed; **box** before store.
    pub fn store_var(
        &mut self,
        name: &str,
        val:  inkwell::values::BasicValueEnum<'ctx>,
        ...
        let (val, stored_ty) = self.maybe_box(val, ty)?;

        let ptr = if let Some(p) = self.get_variable_ptr(name) {
            p
        } else {
            // allocate new alloca in current function entry
            let ptr_t = self.llvm_context.ptr_type(inkwell::AddressSpace::default());
            let new_ptr = self.builder.build_alloca(ptr_t, name)
                .map_err(|e| format!("Failed to allocate variable: {}", e))?;
            self.variables.insert(name.into(), new_ptr);
            self.type_env.insert(name.into(), stored_ty.clone());
            if let Some(scope) = self.scope_stack.current_scope_mut() {
                scope.add_variable(name.into(), new_ptr, stored_ty.clone());
            }
            new_ptr
        };
    ...
    /// Load then **maybe_box()** so callers always get a legal BoxedAny in boxed mode.
    pub fn load_var(
        &mut self,
        name: &str,
    ) -> Result<(inkwell::values::BasicValueEnum<'ctx>, Type), String> {
        let (ptr, ty) = {
            ...
            let ty  = self
                ...
                .ok_or_else(|| format!("unknown type for {}", name))?
                .clone();
            (ptr, ty)
        };
