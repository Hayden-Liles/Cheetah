...
pub fn track_alloc(size: usize, _location: &str) {
    if size >= ALLOCATION_TRACKING_THRESHOLD {
        ...
        while peak < new_usage
            && !PEAK_MEMORY_USAGE
                .compare_exchange_weak(peak, new_usage, Ordering::Relaxed, Ordering::Relaxed)
                .is_ok()
        {
...
pub fn print_memory_stats() {
    ...
    if peak > 0 {
        ...
        if allocs > 0 {
            eprintln!(
                "  Large allocations (>= {} KB): {}",
                ALLOCATION_TRACKING_THRESHOLD / 1024,
                large
            );
        }
    }
...
pub fn register_memory_functions<'ctx>(
    ...
    let track_deallocation_type = context
        .void_type()
