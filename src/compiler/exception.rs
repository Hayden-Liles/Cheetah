...
impl<'ctx> CompilationContext<'ctx> {
    ...
    /// Check if an exception matches a given type
    fn check_exception_type(&self, typ_val: BasicValueEnum<'ctx>) -> inkwell::values::IntValue<'ctx> {
        // Get the exception_check function
        let exception_check_fn = match self.module.get_function("exception_check") {
            Some(f) => f,
            None => {
                // If the function doesn't exist, return true (match all exceptions)
                return self.llvm_context.bool_type().const_int(1, false);
            }
        };
        ...
        // Get the type name from the exception type value
        let type_name = if typ_val.is_pointer_value() {
            let ptr = typ_val.into_pointer_value();
            // Try to get the name from a global variable
            let name_cstr = ptr.get_name();
            if let Ok(name) = name_cstr.to_str() {
                self.create_string_constant(name)
            } else if let Some(func) = self.get_function_from_value(typ_val) {
                // If it's a function, use its name
                let name = func.get_name().to_str().unwrap_or("Exception");
                self.create_string_constant(name)
            } else {
                // Otherwise, use a default name
                self.create_string_constant("Exception")
            }
        } else {
            // If it's not a pointer, use a default name
            self.create_string_constant("Exception")
        };

        // Call exception_check to see if the exception matches this type
        let call_site_value = self.builder.build_call(
            exception_check_fn,
            &[exception.into(), type_name.into()],
            "check_exception",
        ).unwrap();

        call_site_value.try_as_basic_value().left().unwrap().into_int_value()
    }

    /// Get a function from a value (if it's a function pointer)
    fn get_function_from_value(&self, value: BasicValueEnum<'ctx>) -> Option<inkwell::values::FunctionValue<'ctx>> {
        if !value.is_pointer_value() {
            return None;
        }

        // Try to get the function from the module by name
        let ptr = value.into_pointer_value();
        let name_cstr = ptr.get_name();
        if let Ok(name_str) = name_cstr.to_str() {
            return self.module.get_function(name_str);
        }
